...
pressure.new = function (name = "default", config = null) {
    ...
    function check() {
        ...
        if (history.length > instance.config.consecutiveGrowths)
            history.shift();
        tracer.log("Checking memory...", history);
        if (history.length < instance.config.consecutiveGrowths) {
            currentTimeout = setTimeout(check, instance.config.interval);
        ...
        tracer.log("IsConsecutive:%c, isExceeding:%e, isBlocked:%b", isConsecutive, isExceeding, isBlocked);
        //We need to block
        if (isConsecutive && isExceeding) {
            ...
            if (counter == 0 && !isCleared)
                throw new CustomError("No subscription to the event '%s'", UNDER_PRESSURE, 500, "fatal");

            emitter.listeners(UNDER_PRESSURE).forEach(function (listenerFn) {
                let sentObj = {
                ...
                listenerFn.call(instance, sentObj);
            });
        ...
        if (isBlocked && !isExceeding) {
            ...
            emitter.listeners(PRESSURE_RELEASED).forEach(function (listenerFn) {
                let sentObj = {
                    memoryUsageHistory: history,
                    ack: function () {
                        ack();
        ...
        process.nextTick(function () {
            tracer.log("Next check in %i ms", instance.config.interval);
            currentTimeout = setTimeout(check, instance.config.interval);
        });
